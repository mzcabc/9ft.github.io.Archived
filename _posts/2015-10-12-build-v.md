...
###配置 Openswan (IPSEC)
在`/etc/ipsec.conf`中：
修改为`left=%SERVERIP%`、`protostack=netkey`。
`/etc/ipsec.conf`完整文件：
```
...
修改 ipsec.secrets
>- 获取服务器 IP: `curl http://ip.mtak.nl`
>- 生成随机密匙:  `openssl rand -hex 30`

设置IPSec握手时的的 Shared Secret,`/etc/ipsec.secrets`修改为
```
%SERVERIP%  %any:   PSK "%Shared Secret%"
```

验证 IPSEC 是否正常工作 `ipsec verify`

遇到的问题
>- `NETKEY:  Testing XFRM related proc values                 [FAILED]`应该是忘记修改网络策略,解决方法
...
[1] http://phyng.com/digitalocean-ubuntu-vpn.html
[2] http://blog.fengqijun.me/vpn/2015/03/08/l2tp-ipsec-vpn-on-ubuntu-14/
